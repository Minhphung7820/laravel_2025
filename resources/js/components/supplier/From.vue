<template>
  <div class="p-6 bg-white rounded-xl shadow-md w-full">
    <h2 class="text-2xl font-bold mb-6"> {{ this.mode === 'create' ? 'Thêm nhà cung cấp' : 'Chỉnh sửa nhà cung cấp' }}</h2>

    <form @submit.prevent="submitForm" class="grid grid-cols-1 md:grid-cols-2 gap-4" enctype="multipart/form-data">
      <!-- Thông tin cơ bản -->
      <div>
        <label class="block mb-1 text-sm text-gray-700 font-semibold">Tên nhà cung cấp <span class="text-red-500">*</span></label>
        <input
          v-model="form.name"
          type="text"
      ...
      <!-- Phân loại -->
      <div>
        <label class="block mb-1 text-sm text-gray-700 font-semibold">Loại nhà cung cấp</label>
...
export default {
  ...
  async mounted() {
    if (this.mode === 'update' && this.supplierId) {
      try {
        const { data } = await window.axios.get(`/api/supplier/detail/${this.supplierId}`)
